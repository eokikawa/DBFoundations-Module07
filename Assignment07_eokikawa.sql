...
-- 2024-05-28,EmiOkikawa,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_EmiOkikawa')
	 Begin 
	  Alter Database [Assignment07DB_EmiOkikawa] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_EmiOkikawa;
	 End
	Create Database Assignment07DB_EmiOkikawa;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_EmiOkikawa;
...
SELECT
	CategoryName,
	ProductName,
	[UnitPriceUS] = FORMAT (UnitPrice, 'c', 'en-us')
		FROM vProducts
		JOIN vCategories
...
SELECT TOP 1000000
	CategoryName,
	[InventoryDateNew] = CONVERT(varchar(50),(DATENAME(month, InventoryDate) + ', ' + DATENAME(year, InventoryDate))),
...
--simplified date code and changed inventory date column name--
SELECT TOP 1000000
	CategoryName,
...
SELECT
	...
	InventoryDate,
	[Count],
	[PreviousMonthCount] = isNull(Lag(Sum([Count])) OVER(ORDER BY ProductName,month([InventoryDate])),0)
...
SELECT
	...
	InventoryDate,
	[Count],
	[PreviousMonthCount] = isNull(Lag([Count],1) OVER(PARTITION BY ProductName ORDER BY month([InventoryDate])),0)
		FROM vProductInventories
		ORDER BY ProductName, month(InventoryDate)
		go */
